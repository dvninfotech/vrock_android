...
class TrendingFeedsAdapter(
    val context: Context,
    val dataList: ArrayList<Data>,
    val trendingFeedsFragment: TrendingFeedsFragment,
    val itemClickListener: ItemClickListernerWithType
) : RecyclerView.Adapter<TrendingFeedsAdapter.ViewHolder>() {

//    private var myPlayer: MyMultiPlayer? = null
    private var myPlayer: MySinglePlayer? = null

    init {
//        myPlayer = MyMultiPlayer.getInstance(TrendingFeedsFragment.PLAYER_TAG)
        myPlayer = MySinglePlayer.getInstance(TrendingFeedsFragment.PLAYER_TAG)
    }
...
//            Picasso.get().load("http://access.spaceimagingme.com:9090/vdoz/test_thumbail.jpg").into(holder.ivPlaceholder)
