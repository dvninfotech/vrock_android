...
class ForYouFeedsFragment : BaseFragment(), ItemClickListernerWithType,
    SwipeRefreshLayout.OnRefreshListener {
    companion object {
        const val TAG = "ForYouFeedsFragment"
        ...
        var isForYouVisible: Boolean = true
...
//    var myPlayer: MyMultiPlayer? = null
    ...
    var forYouFeedsAdapter: ForYouFeedsAdapter? = null
    ...
    private val forYouViewModel by viewModel<ForYouViewModel>()
...
//        myPlayer = MyMultiPlayer.createInstance(PLAYER_TAG, context!!)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        forYouFeedsAdapter = ForYouFeedsAdapter(context!!, feedsData, this, this)

        rvFeeds.layoutManager = layoutManager
        rvFeeds.adapter = forYouFeedsAdapter
...
//                        myPlayer?.resumeCurrentPlayer()
    ...
    private fun onFeedsChanged() {
        try {
            ...
            forYouFeedsAdapter?.notifyDataSetChanged()
        } catch (exp: Exception) {
        ...
        if (totalItemCount > 0 && endHasBeenReached) {
            if (!isNextPageLoading) {
                ...
                if (VrockkApplication.isLoggedIn()) {
                    forYouViewModel.getForYouFeeds(
                        getMyAuthToken(),
                        playingFeedsPageNo,
    ...
    private fun addObservers() {
        forYouViewModel.getForYouResponseLiveData()
        ...
        forYouViewModel.errorMessage.observe(activity!!, Observer { it ->
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == 20) {
            if (data?.hasExtra("count")!!) {
                ...
                forYouFeedsAdapter?.notifyItemChanged(position)
            }
