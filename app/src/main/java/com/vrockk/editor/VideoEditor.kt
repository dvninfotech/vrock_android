...
class VideoEditor private constructor(private val context: Context) {
    ...
    fun main() {
        ...
        try {
            ...
            Log.e("EXCECUTION"," list ${  FFmpeg.listExecutions()}")
            FFmpeg.cancel()
            val executionId = FFmpeg.executeAsync(cmd) { executionId, returnCode ->
                if (returnCode == RETURN_CODE_SUCCESS) {

                    when(type){

                    Constant.AUDIO_TRIM -> callback!!.onSuccess(outputFile, OutputType.TYPE_AUDIO)

                    Constant.VIDEO_AUDIO_MERGE -> callback!!.onSuccess(outputFile, OutputType.TYPE_VIDEO)

                    Constant.VIDEO_PLAYBACK_SPEED -> callback!!.onSuccess(outputFile, OutputType.TYPE_VIDEO_SPEED)

                    Constant.MERGE_VIDEO -> callback!!.onSuccess(outputFile, OutputType.TYPE_MERGE_VIDEO)
                }
                    Log.i(
                        Config.TAG,
                    ...
                    Log.i(
                        ...
                        java.lang.String.format(
                            "Async command execution failed with "            )
                    )
                }
            }
        ...
        Log.e("EXCEPTION", "video editor $e")
    }
    }
}